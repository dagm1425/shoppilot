...
class _FakeCompletions:
    ...
    def create(self, **kwargs):  # noqa: ANN003, ANN204
        if self._should_raise:
            raise RuntimeError('synthetic llm failure')
        if self._content is None:
            raise RuntimeError('missing synthetic content')
        return _FakeResponse(self._content)


class _FakeClient:
    def __init__(self, *, content: str | None = None, should_raise: bool = False) -> None:
        self.chat = type('Chat', (), {'completions': _FakeCompletions(content=content, should_raise=should_raise)})()

    def with_options(self, **kwargs):  # noqa: ANN003, ANN204
        return self

...
def test_settings_allow_langsmith_disabled_without_optional_keys(
    ...
    assert settings.langchain_project == 'Shoppilot'
...
def test_settings_apply_llm_synthesis_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert settings.ai_llm_synthesis_timeout_ms == 18000
    assert settings.ai_llm_synthesis_max_tokens == 800
    assert settings.ai_llm_synthesis_temperature == 0.2
    assert settings.ai_llm_synthesis_top_n_products == 5
...
def test_settings_support_deprecated_openai_synthesis_aliases(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path,
) -> None:
...
def test_settings_reject_openai_compatible_llm_endpoint_in_production(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _base_env(monkeypatch)
...
def test_settings_allow_gemini_llm_endpoint_in_production(monkeypatch: pytest.MonkeyPatch) -> None:
    _base_env(monkeypatch)
    monkeypatch.setenv('NODE_ENV', 'production')

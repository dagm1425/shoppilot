...
const envSchema = z.object({
  ...
  AI_GATEWAY_TIMEOUT_MS: z.coerce.number().int().positive().default(25_000),
  AI_RATE_LIMIT_TTL_SEC: z.coerce.number().int().positive().default(60),
  AI_RATE_LIMIT_MAX: z.coerce.number().int().positive().default(8),
  DATABASE_URL: z.string().url().default('postgresql://postgres:postgres@localhost:5432/shoppilot'),
  ...
  QUEUE_PREFIX: z.string().trim().min(1).default('shoppilot'),
  ORDER_CONFIRMATION_JOB_ATTEMPTS: z.coerce.number().int().positive().default(3),
  ORDER_CONFIRMATION_JOB_BACKOFF_MS: z.coerce.number().int().positive().default(30_000),
